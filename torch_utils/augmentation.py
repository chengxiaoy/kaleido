...
def matrix(*rows, device=None):
    ...
    elems = [x if isinstance(x, torch.Tensor) else misc.constant(x, shape=ref[0].shape, device=ref[0].device) for x in
             elems]
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if self.imgfilter > 0:
            ...
            for i, band_strength in enumerate(self.imgfilter_bands):
                ...
                t_i = torch.where(torch.rand([batch_size], device=device) < self.imgfilter * self.p * band_strength,
...
if __name__ == '__main__':
    aug_params = dict(xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=1, hue=1, saturation=1)
    ap = AugmentPipe(**aug_params)
