...
class ALI(BaseVAE):
    ...
    def forward(self, x: Tensor, **kwargs) -> Tensor:
        ...
        z_ = self.reparameter_trick(mu, logvar)

        z = torch.randn(z_.shape).to(x.device)
        x_ = self.decode(z)

        real_label = self.discriminator(x, z_)
        fake_label = self.discriminator(x_, z)

        return [x, x_, z, z_, real_label, fake_label, labels]
    ...
    def loss_function(self, *inputs: Any, **kwargs) -> Tensor:
        ...
        # print("real=======================")
        # print(real_label)
        gan_d_loss = nn.BCEWithLogitsLoss()(fake_label, labels.clone().fill_(fake)) \
                     + nn.BCEWithLogitsLoss()(real_label, labels.clone().fill_(real))

        gan_g_loss = nn.BCEWithLogitsLoss()(fake_label, labels.clone().fill_(real)) \
                     + nn.BCEWithLogitsLoss()(real_label, labels.clone().fill_(fake))

        return {"loss": gan_g_loss + gan_d_loss, "gan_g_loss": gan_g_loss, "gan_d_loss": gan_d_loss}
